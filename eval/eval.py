...
def copy_csv_to_evaluation(
    ...
    # Add all new columns at once (avoids fragmented DataFrame and performance warnings)
    nrows = len(df)
    to_add = {}
...
def semantic_judge_for_model(model: str, user_message: str) -> str:
    ...
    if model == "codellama-7b":
        return models.Codellama7b(preprompt, user_message)
    if model == "codellama-34b":
        return models.Codellama34b(preprompt, user_message)
    if model == "Wizardcoder33b":
        return models.Wizardcoder33b(preprompt, user_message)
...
def model_evaluation(
    ...
    for i in range(NUM_EXISTING_SAMPLES, NUM_SAMPLES_PER_TASK):
        ...
        while True:
            ...
            if model == "gpt-5.3-codex":
                text = models.GPT5_3_Codex(preprompt, prompt, gpt_client)
            elif model == "gpt-5.4":
                text = models.GPT5_4(preprompt, prompt, gpt_client)
                ...
                df.at[index, "LLM DEVS Correct? #" + str(i)] = "Failure"
            ...
            devs_eval_result = x.get("devs_evaluation_result")
            if devs_eval_result is None:
                legacy_result_key = next(
                    (key for key in x if key.endswith("_evaluation_result")), None
                )
                if legacy_result_key:
                    devs_eval_result = x.get(legacy_result_key)

            devs_eval_error = x.get("devs_evaluation_error")
            if devs_eval_error is None:
                legacy_error_key = next(
                    (key for key in x if key.endswith("_evaluation_error")), None
                )
                if legacy_error_key:
                    devs_eval_error = x.get(legacy_error_key)
            df.at[index, "LLM Plannable? #" + str(i)] = x["devs_plan_success"]
            df.at[index, "LLM DEVS Correct? #" + str(i)] = devs_eval_result
            df.at[index, "LLM Plan Phase Error #" + str(i)] = x["devs_plan_error"]
            df.at[index, "LLM DEVS match phase Error #" + str(i)] = devs_eval_error
            df.at[index, "LLM Notes #" + str(i)] = x["notes"]
            logging.info("Plan Result Summary:")
            for key, value in x.items():
                logging.info(f"{key}: {value}")

            if devs_eval_result == "success":
                num_correct += 1
                break
                ...
                elif devs_eval_result == "Failure":
                    prompt = prompt_templates.multi_turn_rego_error_prompt(
                        row["prompt"], code, policy_file, devs_eval_error
                    )
                continue
            else:
                break
