...
import math
import json
import rodada2.metrics as metrics
import eval
import rodada2.models as models
...
DELIMITERS = ["```devs", "```DNL", "```dnl", "```SES", "```ses", "```json", "```"] # ``` needs to be in the end, as it is a final "else" case
...
logger = logging.getLogger("devs-eval-metric")
...
def copy_csv_to_metric(base_eval_files, new_base_metric_dir, llm="gpt4"):
    """
        Example:
            base_eval_files: ["../results-for-devs-eval (backup)/george-existing/evaluation-dataset-george-gpt3.5.csv"]
            llm_base_eval_dir: "../results-for-devs-eval (backup)/george-existing/llm-judge-evaluation-metric"
    """
    dst_filenames = []
...
def main(): # E.g.,: python3 llm-judge-eval.py gpt4 llm-single
    ...
    # base_eval_files = ['/home/ubuntu/devs-tasks/evaluation/../results-for-devs-eval (backup)/multi-turn/gpt4/lei-existing/evaluation-dataset-multi-turn-lei-gpt4-test-1.csv'] # just for debugging purposes
